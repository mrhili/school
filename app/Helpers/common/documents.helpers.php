...
class Documents {
  ...
  public static function getDoc( $model_type, $pdf_id , $add_info = null ){
    ...
    if( $pdf ){
      ...
      $path = public_path().'/documents/'. ArrayHolder::modelTypes4file($model_type)['folder'] .'/' ;
      $asset_path = '/documents/'. ArrayHolder::modelTypes4file($model_type)['folder'] .'/' ;
      ...
      if($model_type == 0){
        ...
        if( file_exists( $path.$pdf->name.'.'.$pdf->ext ) ){
          ...
          return asset('/documents/config/no-doc2.doc' );
        }
  ...
  public static function storeDocs( $request , $model_type, $model , $add_info = null ){
