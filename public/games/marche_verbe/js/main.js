...
function cargarjuego() {
  ...
  console.log("Cargando juego");
    ...
    $.getJSON( "/games/marche_verbe/verbos/verbs.json", function( data ) {
      //console.log(data);
...
function linksMenu() {
  ...
  if (bloqueoMenu==false)  {
    $("#img_opt1").attr("src","/games/marche_verbe/img/opt1_inactivo.png");
    $("#img_opt2").attr("src","/games/marche_verbe/img/opt2_inactivo.png");
    $("#img_opt3").attr("src","/games/marche_verbe/img/opt3_inactivo.png");

    $(".options").mouseenter(function () {
      $("#img_"+this.id).attr("src","/games/marche_verbe/img/"+this.id+"_activo.png");
    });
    $(".options").mouseleave(function () {
      $("#img_"+this.id).attr("src","/games/marche_verbe/img/"+this.id+"_inactivo.png");
    });
    ...
    $("#img_opt1").attr("src","/games/marche_verbe/img/opt1_gris.png");
    $("#img_opt2").attr("src","/games/marche_verbe/img/opt2_gris.png");
    $("#img_opt3").attr("src","/games/marche_verbe/img/opt3_gris.png");
  }
};
...
function ejecutarMusicaFondo() {
  ...
  if (audioOn) {
    ...
    sonidoFondo.play();
    $("#img_interruptor_Audio").attr("src", "/games/marche_verbe/img/icon_volumen1.png");
...
function interruptorAudio() {
  if (audioOn) {
    $("#img_interruptor_Audio").attr("src", "/games/marche_verbe/img/icon_volumen2.png");
...
function cargarFondos() {
  ...
  switch (modo) {
    case 1:
      $("#contenedor_fondos").attr("src","/games/marche_verbe/img/fondo_pantalla_spanishpair.jpg");
    break;
    case 2:
      $("#contenedor_fondos").attr("src","/games/marche_verbe/img/fondo_pantalla_futuro.jpg");
    break;
    case 3:
      $("#contenedor_fondos").attr("src","/games/marche_verbe/img/fondo_pantalla_pasado.jpg");
    break;
    default:
...
function cargarBotonesAccesos() {
  $("#btnAyuda").attr("src","/games/marche_verbe/img/btn_ayuda_inactivo.png");
  $("#btnAyuda").attr("alt","Help");
  $("#btnMenu").attr("src","/games/marche_verbe/img/btn_menu_inactivo.png");
  $("#btnMenu").attr("alt","Menu");
  $("#btnAcerca").attr("src","/games/marche_verbe/img/btn_acerca_de_inactivo.png");
...
function cargarAudioVerbos(lista) {
  switch (modo) {
    case 1:
      ...
      $(".grupo_audios").append("<audio class='audios' preload='auto' src='/games/marche_verbe/audio/"+ modificarC.getNombreArchivo(lista[i].pre) +".mp3'  id='"+ modificarC.getNombreArchivo(lista[i].pre) +"'></audio>");
    ...
    case 2:
      ...
      for (var i = 0; i < totalCartas; i++) {
      $(".grupo_audios").append("<audio class='audios'  preload='auto' src='/games/marche_verbe/audio/"+ modificarC.getNombreArchivo(lista[i].pre) +".mp3'  id='"+ modificarC.getNombreArchivo(lista[i].pre) +"'></audio>");
      $(".grupo_audios").append("<audio class='audios'  preload='auto' src=/games/marche_verbe/'audio/"+ modificarC.getNombreArchivo(lista[i].fut) +".mp3'  id='"+ modificarC.getNombreArchivo(lista[i].fut) +"'></audio>");
    }
    break;
    case 3:
      ...
      for (var i = 0; i < totalCartas; i++) {
        ...
        $(".grupo_audios").append("<audio class='audios'  preload='auto' src='/games/marche_verbe/audio/"+ modificarC.getNombreArchivo(lista[i].pre) +".mp3'  id='"+  modificarC.getNombreArchivo(lista[i].pre) +"'></audio>");
        $(".grupo_audios").append("<audio class='audios'  preload='auto' src='/games/marche_verbe/audio/"+ modificarC.getNombreArchivo(lista[i].pas) +".mp3'  id='"+ modificarC.getNombreArchivo(lista[i].pas) +"'></audio>");
      }
      break;
  default:
  }
...
function cargarCartasDer(lista) {
  ...
  switch (modo) {
    ...
    case 2:
      // modo cartas futuro
    for (var i = 0; i < lista.length; i++) {
        $("#contendorDerecho").append("<div class='cartas cartas_izq cartas_derecha'  nombreaudio = '"+ modificarC.getNombreArchivo(lista[i].fut) +"'  id='izq"+lista[i].id+"'>"+ lista[i].fut +"</div>");

    };
    break;
    case 3:
    // modo 3 cartas pasado
    for (var i = 0; i < lista.length; i++) {
        $("#contendorDerecho").append("<div class='cartas cartas_izq cartas_derecha' nombreaudio = '"+ modificarC.getNombreArchivo(lista[i].pas) +"' id='izq"+lista[i].id+"'>"+ lista[i].pas +"</div>");
...
function acomodarCartasGanadas() {
  ...
  var p1 = $("#"+seleccionadaIng.id).html();
  var p2 = $("#"+seleccionadaEsp.id).html();
  ...
  $("#aEsp"+consecutivoGanada).html(p2);
  $("#aIng"+consecutivoGanada).html(p1);
  //Efecto animación:
...
function mostrarFinJuego() {
  ...
  if (timerOn) {
    ...
    if (tiempoWorker >= 200) {
      $("#imagen_modal").attr("src","/games/marche_verbe/img/bronce.jpg");
      $("#imagen_modal").attr("alt","Bronze medal");
    };
    if (tiempoWorker >= 186 &&  tiempoWorker <= 199) {
      $("#imagen_modal").attr("src","/games/marche_verbe/img/plata.jpg");
      $("#imagen_modal").attr("alt","Silver Medal");
    };
    if (tiempoWorker <= 185) {
      $("#imagen_modal").attr("src","/games/marche_verbe/img/oro.jpg");
      $("#imagen_modal").attr("alt","Gold Medal");
    }

  }else {
      $("#imagen_modal").attr("src","/games/marche_verbe/img/fondo_menu.jpg");
    ...
    $("#reloj").css("visibility", "visible");
      if(typeof(Worker) !== "undefined") {
          if(typeof(w) == "undefined") {
              w = new Worker("/games/marche_verbe/js/reloj.js");
          }
...
function agregarClicInterruptorTiempo() {
  if (timerOn) {
    $("#imgSwitchTimer").attr("src","/games/marche_verbe/img/img_timer_off.png");
  }else {
    $("#imgSwitchTimer").attr("src","/games/marche_verbe/img/img_timer_on.png");
  };
  timerOn=!timerOn;
  console.log(timerOn);
}
